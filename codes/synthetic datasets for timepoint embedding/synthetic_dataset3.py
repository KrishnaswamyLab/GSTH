...
simulate_signal_ = []
simulate_signal_.append(toy_feature_)
temp = toy_feature_
for i in range(300):
    temp = np.matmul(M,temp)
    simulate_signal_.append(temp)


simulate_signal = np.array(simulate_signal)
simulate_signal_ = np.array(simulate_signal_)


simulate_signal_com = np.concatenate((simulate_signal,simulate_signal_),0)
...
simulate_signal_com = np.array(simulate_signal_com).transpose()
feature_z = scipy.stats.mstats.zscore(simulate_signal_com,0)
scattering_feature_ = generate_mol_feature(adj,feature_z)
scattering_feature_ = scattering_feature_.transpose()


phate_operator1 = phate.PHATE(n_components=3,knn=20)
phate_data1 = phate_operator1.fit_transform(scattering_feature_) 
...
pca_feature = PCA(n_components=3).fit_transform(scattering_feature_)
fig = plt.figure(figsize=(16,8))
ax1 = plt.subplot2grid(shape=(1,1), loc=(0,0),projection='3d')
...
feature_tsne= TSNE(n_components=3).fit_transform(scattering_feature_)
fig = plt.figure(figsize=(16,8))
ax1 = plt.subplot2grid(shape=(1,1), loc=(0,0),projection='3d')
...
phate_data2 = phate_operator2.fit_transform(simulate_signal_com.transpose()) 
fig = plt.figure(figsize=(16,8))
ax1 = plt.subplot2grid(shape=(1,1), loc=(0,0),projection='3d')
...
umap_feature = umap_operator.fit_transform(scattering_feature_)
fig = plt.figure(figsize=(16,8))
ax1 = plt.subplot2grid(shape=(1,1), loc=(0,0),projection='3d')
...
#an example of calculating persistence diagram based on umap trajectories
